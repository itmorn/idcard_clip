...
import warnings
import random
from a01_location7 import cfg, utils_calc,util_dlib
...
class StepA:
    ...
    def run(self):
        ...
        if self.error_rect_best > cfg.th_error_rect or self.error_var_angle_best > cfg.th_var_error:
            return RES.NO_PASS, RES.ERROR_INFO_DISTORTION
            # print("照片畸变")
            # self.save_pic(cfg.dir_distortion, self.img_match_best)
            # return
        # cv2.imwrite("/data/zhaoyichen/filename.png", self.img_test_res)
        # 透视变换
        self.perspective_transform()

        img_gray = cv2.cvtColor(self.img_test_res, cv2.COLOR_RGB2GRAY)
        lst_detected_face =util_dlib.get_lst_detected_face(img_gray)
        if len(lst_detected_face)==0:
            return RES.NO_PASS, RES.ERROR_INFO_VAGUE

        # 移除左右边框
        self.remove_border_by_face(lst_detected_face[0])
        ...
        return RES.PASS,self.img_test_res
    ...
    def remove_border(self):
        ...
        # w_output = 600  # 532
        # self.img_test_res = cv2.resize(self.img_test_res, (532, 337))
        ...
        right = cfg.w_output-40  # 右边从多少像素开始判断边框
        # 将头像覆盖
        img_expand = cfg.remove_portrait(img_expand)
    ...
    def remove_border_by_face(self, detected_face):
        face_keypoint = util_dlib.get_face_keypoint(self.img_test_res,detected_face, util_dlib.predictor5).tolist()
        top,bottom,left,right = detected_face.top(), detected_face.bottom(), detected_face.left(), detected_face.right()
        # right_eye = face_keypoint[0][0]
        # left_eye = face_keypoint[2][0]
        # center = (left_eye+right_eye)/2
        # # 右边界 为 中心线到右眼睛的距离的N倍
        # N = 3.8
        # harf = right_eye - center
        # right = int(harf * N + center)
        self.img_test_res = self.img_test_res[:, :right+70]
        return
        # cv2.imwrite("/data/zhaoyichen/filename.png", canny.astype("uint8") * 255)
...
def f(url_img):
    ...
    if min(h, w) < cfg.min_h_or_w:  # 舍弃尺寸较小的照片
        return RES.NO_PASS, RES.ERROR_INFO_SIZE

    stepA = StepA(url_img, img_test_rgb).run()
    return stepA
...
if __name__ == '__main__':
    ...
    dir_input = "../img_idcard/" #第二批 2880
    lines = [dir_input + i for i in os.listdir(dir_input)][10]
